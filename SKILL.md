...
当查询医生介绍时，脚本返回的数据中会包含 `image_path` 字段。

**必须同时展示医生图片和文字介绍：**

1. 先展示医生图片（读取 `image_path` 指向的文件）
2. 在图片下方展示文字介绍（姓名、职称、描述）

示例回复格式：

```
[展示图片: assets/images/wu-lingyan.jpg]

吴凌燕 - 院长
创始人、北京首批美容皮肤科主诊医师、《面部危险区》译者、各大品牌导师，荣誉众多
```
